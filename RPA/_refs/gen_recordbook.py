...
REVISION_HISTORY = [
    ("v2.3", "2026-09-06",
     "随 08_ 纸上篇 v1.2（案例包+分次揭晓）对齐：A 页④改「十二个货各归哪类」（石家庄 12 货号码填五类＋每类配私例，"
     "分对 8 个过关）；B 页①②③改案例版（①食堂叫号屏拆三要素②打印店补命令＋揪 AI 卧底③正定行程单挑刺＋夜市循环挑战）；"
     "各页与课件一一对应。另：本文件及全部新文档此前未真正入库（add 静默失败），本次逐个验证入库。"),
    ("v2.2", "2026-09-06",
     "B 页暂停点③随 08_ 纸上篇 v1.1 改为「挑刺：正定一日游的命令」（接力赛拼卡活动取消——全书 32 课时，"
     "轻活动重案例，硬规矩⑪）；其余页不动，已印的无需重印。"),
    ("v2.1", "2026-09-06",
     "B 页暂停点②本地化修订：地铁票价（石家庄地铁实为全程 2 元，与题不符）改为石家庄出租车真实计价"
     "（起步 3 公里 8 元、超出每公里 1.6 元）；随 08_ 纸上篇插图版（91-D3 主线全用图）同步。"),
    ("v2.0", "2026-09-06",
     "按教师定盘「基础不牢不碰软件」重构：第 2/3/4 次课全部纸面（合并课件 08_纸上篇），第 5 次课智多星才登场。"
     "追加页重排：A＝第 2 次课变量（⑤悬念揭晓改为「第 5 次课的答案」）；B＝第 3 次课命令（①三要素②串法对号＋家例"
     "③命令卡互审挑刺④三句＋私例⑤外卖图纸）；C＝第 4 次课流程与调试（①图纸三行②人肉运行记录③验收单）；"
     "D＝第 5 次课机器人上岗（①打招呼顺序②先猜后验③读报错④排错三行⑤结业三句＋真机例子）。"
     "第 1 页不动，已印的无需重印；A 页若已印可手改⑤的「下节课」为「第 5 次课」。"),
    ("v1.9", "2026-09-06",
...
pause_point("④", "十二个货，各归哪类？",
            "把①–⑫的号码填进五类：①饭卡 35.5 ②气温 26 ③隆兴寺预约号 ④地铁票价 2 元 ⑤骑手位置 ⑥借书到期日 ⑦取件码 ⑧到站时间 ⑨学生证照片 ⑩「运输中」 ⑪大锅菜 6 元 ⑫微信步数——每类再配一个自己的新例子。分对 8 个以上过关。",
            labels=("数值", "字符", "日期/时间/文件"))

pause_point("⑤", "今天的争论题：取件码 A8-2012 能当数算吗？",
            "写下你的判断＋一条理由。今天不揭晓——第 5 次课让真机器人跑一遍，把答案抄回来。",
            labels=("我的判断", "我的理由", "第 5 次课的答案"))
...
# ================= 追加页 B：第 3 次课 · 项目二第 2 讲（v2.0 改版·命令纸面） =================
doc.add_page_break()
para("第 3 次课 · 项目二第 2 讲　教机器人干活——命令　　　　日期：＿＿＿＿＿＿　　互审搭档：＿＿＿＿",
     size=12, bold=True, name="黑体", align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)
para("教室课（命令卡）。先猜再验；看到课件「📒暂停点N」才动笔。上机统一在第 5 次课。",
     size=9, color=GRAY, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)

pause_point("①", "案例①·食堂叫号屏：拆三要素",
            "「37 号同学，你的麻辣烫好了」——动作＝让屏幕干什么？参数＝给了什么要求？输出＝大家看到什么？追问：只弹话不弹号，窗口会怎样？",
            labels=("动作", "参数", "输出"))

pause_point("②", "案例②·打印店：补命令＋揪卧底",
            "①把「打印讲义」补成师傅一句话都不用问的完整命令；②4 条 AI 说明里哪条说反了？写一条理由（尺子：参数＝我要什么，输出＝它给什么）。",
            labels=("我的完整命令", "卧底是＿条", "我的理由"))

pause_point("③", "案例③·正定行程单挑刺",
            "38 人的一日游三行命令——最要命的缺参数是哪处？缺了周六早上会出什么乱子？挑战：给夜市加一条循环命令。",
            labels=("缺的参数", "会出的乱子", "我的补法/挑战"))

pause_point("④", "收尾三句＋你的私例",
            "①命令＝动作＋参数＋输出　②三种串法：顺序/条件/循环　③参数缺一条，命令就歪——每句配一个你自己的例子。",
            labels=("三句话抄下来", "我的例子", "同桌判"))

pause_point("⑤", "期末图纸：外卖满减计算器（单价 39.9、买 2 份、满 25 减 4）",
            "①要哪几个变量（名字＋类型）②哪几条命令 ③用哪种串法。下节课「人肉运行」照图纸开工——图纸不全不开工。",
            labels=("变量清单", "命令清单", "串法"))
...
r = p.add_run("下课前自查：")
set_font(r, size=10, bold=True, name="黑体", color=ORANGE_DARK)
r2 = p.add_run("□ ①我会拆三要素　□ ②我对上串法　□ ③我挑出缺参数　□ ④三句配了私例　□ ⑤图纸画全了　　"
               "家里例子最妙的一个：＿＿＿＿＿＿＿＿＿＿")
set_font(r2, size=10, name="宋体")

# ================= 追加页 C：第 4 次课 · 项目二第 3 讲（v2.0 改版·流程纸面） =================
doc.add_page_break()
para("第 4 次课 · 项目二第 3 讲　跑起来修得好（纸上篇）　　　　日期：＿＿＿＿＿＿　　互审搭档：＿＿＿＿",
     size=12, bold=True, name="黑体", align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)
para("教室课（人肉运行）。没图纸不开工；卡住三板斧：读现象→找那一步→问同桌。",
     size=9, color=GRAY, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)

pause_point("①", "图纸三行（补全＋同桌点头才开工）",
            "①变量清单（名字＋类型）②命令清单 ③串法。讲给同桌听一遍，他点头才开工。",
            labels=("变量清单", "命令清单", "串法"))

pause_point("②", "人肉运行记录：今天卡得最狠的那次",
            "一人当机器人照你的图纸执行、一人当主管记卡壳。写全四行＝一张收网记录。",
            labels=("现象（卡在哪步）", "我改了图纸哪行", "再「跑」怎样"))

pause_point("③", "主管验收单",
            "他的图纸人肉跑给你看，再出个刁钻输入考它：0 份？恰好 25 元？负数？翻车的帮他指到那一步。",
...
r = p.add_run("下课前自查：")
set_font(r, size=10, bold=True, name="黑体", color=ORANGE_DARK)
r2 = p.add_run("□ ①图纸点头了　□ ②我写满收网记录　□ ③我出了刁钻输入　　"
               "今天修掉的最狠的 bug：＿＿＿＿＿＿＿＿＿＿")
set_font(r2, size=10, name="宋体")

# ================= 追加页 D：第 5 次课 · 项目二第 4 讲（v2.0 新增·机器人上岗） =================
doc.add_page_break()
para("第 5 次课 · 项目二第 4 讲　机器人上岗（智多星初见）　　　　日期：＿＿＿＿＿＿　　互审搭档：＿＿＿＿",
     size=12, bold=True, name="黑体", align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)
para("机房课。先纸后机：今天每个按钮，都是你纸上会过的。报错先自己读 3 秒再举手。",
     size=9, color=GRAY, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2)

pause_point("①", "《打招呼》里，机器人依次干了哪几件事？",
            "按顺序写（①②③…），一件一行。写完和同桌对：和你的命令卡排的一样吗？")

pause_point("②", "先猜后验：放 50→输出，再放 200→输出——屏幕先后打出什么？50 和 200 都印吗？",
            "先写猜想再点运行。猜错不扣分——猜错的地方正是要点。",
            labels=("我的猜想", "验证之后", "老师的讲解"))

pause_point("③", "读报错：A8-2012 + 10 的报错原话（两节课前的悬念今天揭晓）",
...
pause_point("⑤", "结业三句＋今天的真机例子",
            "①会记（变量：建/放/换/取）②会派（命令：三要素＋三种串法）③会修（调试：读→定位→改）——每句配一个今天真机上的例子。",
            labels=("会记·例子", "会派·例子", "会修·例子"))
...
r = p.add_run("下课前自查：")
set_font(r, size=10, bold=True, name="黑体", color=ORANGE_DARK)
r2 = p.add_run("□ ①我按顺序写全　□ ②我先猜后验　□ ③我抄了报错原话　□ ④我写满排错三行　□ ⑤三句配了例子　　"
               "我最怕的报错词：＿＿＿＿＿＿＿＿＿＿")
set_font(r2, size=10, name="宋体")
