...
pause_point("①", "「弹出提示」的三要素",
            "老师投的命令卡——动作＝干什么；参数＝按什么要求；输出＝出什么结果。空参数那次算不算有输出？写一句理由。",
            labels=("动作", "参数", "输出"))

pause_point("②", "三种串法对号入座＋你家的例子",
            "洗衣机（洗→漂→甩干）、地铁（超 10 公里 5 元、否则 3 元）、跑圈（满 3 圈才停）各是哪种串法？再各写一个你家里的例子，同桌互判。",
            labels=("顺序", "条件", "循环"))

pause_point("③", "命令卡互审：挑刺",
            "交换同桌用命令卡拼的流程——哪张卡缺参数？缺了会怎样？写下来。",
            labels=("缺的参数", "缺了会怎样", "我的建议"))
