...
def gen_recordbook():
    ...
    para(doc, "项目一 · 第2讲：刁难任务卡·第一轮｜课件 06", 14, True, (0xBF, 0x36, 0x0C))
    pause_block("⑤", "两种问法：老问法 vs 模板", 8,
                "原来的问法和填了模板的问法，差在哪？刚才修漏项，你用的是哪一句？",
                deck="06")
    pause_block("⑥", "让 AI 听话的那一步", 11,
                "今天，你是用哪一步让 AI 听话的？一句话写下来。",
                deck="06")
    para(doc, "✅ 下课前自查：□⑤我写出了两种问法的差别　□⑥我写出了起效的那一步　"
              "□今天三张卡我做过了：＿＿张　□小抄已截图保存", 11)

    para(doc, "项目一 · 第3讲：让AI给你画画拍视频｜课件 07", 14, True, (0xBF, 0x36, 0x0C))
    pause_block("⑦", "我的P图指令", 5,
                "你的 P 图指令，原样写下来；调满意的那一句，也写下来。",
                deck="07")
    pause_block("⑧", "让图动起来的那句话", 9,
                "为了让视频动得自然，你那句话里多说了什么？（动作？快慢？镜头往哪走？）",
                deck="07")
    para(doc, "✅ 下课前自查：□⑦我写出了P图指令　□⑧我写出了动法描述那句　"
              "□把『带声音的视频』发群里一份（结课展素材）", 11)

    para(doc, "项目二 · 第4讲：给AI发一本参考书（ima 知识库）｜课件 08", 14, True, (0xBF, 0x36, 0x0C))
    pause_block("⑨", "有参考书，答案就不一样", 8,
                "ima 的答案，跟热身时豆包的答案，差在哪？这个答案是从哪来的？",
                deck="08")
    pause_block("⑩", "5 道题，验出什么", 10,
                "5 题，ima 答出来几题？哪一题你差点就信了，结果跟原文对不上？",
                deck="08")
    pause_block("⑪", "AI 的花活，有没有错", 13,
                "三个花活（小测验 / 思维导图 / 播客）你体验了哪个？AI 生成的东西里，你发现错了吗？",
                deck="08")
    para(doc, "✅ 下课前自查：□⑨我说清了知识库为什么管用　□⑩我记下了 5 题的对错　"
              "□⑪我体验了一个花活　□班级共享库我扫码加入了", 11)

    para(doc, "项目三 · 第5讲：教AI看说明书（技能）｜课件 09", 14, True, (0xBF, 0x36, 0x0C))
    pause_block("⑫", "我调用技能的那句命令", 9,
                "你调用技能的那句命令，原样抄下来；技能名也写上。",
                deck="09")
    pause_block("⑬", "没技能和有技能，差在哪", 11,
                "没技能的 AI 和有技能的 AI，差在哪？一句话写下来。",
                deck="09")
    para(doc, "✅ 下课前自查：□⑫我抄下了调用命令　□⑬我说清了差在哪　"
              "□技能干完活的文件截图发群了", 11)

    para(doc, "项目三 · 第6讲：召唤专家团，让智能体按时上班｜课件 10", 14, True, (0xBF, 0x36, 0x0C))
    pause_block("⑭", "哪一样，单个AI最难干好", 6,
                "专家团的三样结果里，哪一样最难让一个 AI 单独干好？为什么？一句话。",
                deck="10")
    pause_block("⑮", "三个定时任务", 11,
                "三个定时任务各是几点响、干什么？哪一个你最想收到？抄下来。",
                deck="10")
    para(doc, "✅ 下课前自查：□⑭我写出了哪样最难　□⑮我抄下了三个定时任务　"
              "□晚上的定时推送，我截图发群了（+1）", 11)

    doc.save(os.path.join(OUT_DIR, f"03_我的AI实验记录_全程_v1.9_20260909.docx"))
    print("saved 03")
